...
public interface DeclarationJpaRepository extends JpaRepository<DeclarationModel, Integer> {
    ...
    @Query(value = """
            ...
            declaration_count AS (
                ...
                WHERE fd.declaration_status <> 'vacation'
            ),
            remote_count AS (
    ...
    @Query(value = """
              ...
              vacation_count AS (
                SELECT fd.declaration_user_id, count(*) as vacation_count
                FROM filtered_declarations fd
                WHERE fd.declaration_status = 'vacation'
                GROUP BY fd.declaration_user_id
              )
              ...
              SELECT DISTINCT fu.user_id, fu.user_first_name, fu.user_last_name,
                ...
                COALESCE(rc.remote_count, 0) as remote_count,
                COALESCE(oc.onsite_count, 0) as onsite_count,
                COALESCE(vc.vacation_count, 0) as vacation_count
              ...
              LEFT JOIN vacation_count vc ON fu.user_id = vc.declaration_user_id
            """, nativeQuery = true)
    ...
    @Query(value = """
              ...
              LEFT JOIN vacation_count vc ON fu.user_id = vc.declaration_user_id
            """, nativeQuery = true)
    List<Object[]> getPerTeammateStatisticsByCustomerAndTeamId(LocalDate periodStart, LocalDate periodEnd, int customerId, int teamId);
    ...
    @Query(value= """
            ...
            vacation_count AS (
            	SELECT fd.week_day, count(*) as vacation_count
            	FROM filtered_declarations fd
            	WHERE fd.declaration_status = 'vacation'
            	GROUP BY fd.week_day
            )
            ...
            LEFT JOIN vacation_count vc ON dc.week_day = vc.week_day
