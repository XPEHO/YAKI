...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'avatar_enum') THEN
        CREATE TYPE avatar_enum AS ENUM ('avatarH', 'avatarF', 'avatarN', 'avatarNone', 'userPicture');
    END IF;
END
$$;
...
(
    ...
    user_avatar_choice integer NOT NULL, 
...
CREATE TABLE IF NOT EXISTS public.avatar(
    ...
    avatar_reference avatar_enum NOT NULL,
    avatar_blob bytea,
...
ADD COLUMN IF NOT EXISTS user_avatar_choice INTEGER DEFAULT 1;