...
describe('DeclarationService', () => {
  ...
  describe('updateDeclarationStatus', () => {
   ...
   if the declaration does not exist. */
    it('should throw an error if declaration does not exist', async () => {
      ...
      jest
        .spyOn(declarationRepository, 'getDeclarationById')
        .mockResolvedValueOnce(Promise.resolve<any>(null));

      await expect(
        declarationService.updateDeclarationStatus(declarationId, declaration)
      ).rejects.toThrow(TypeError);
    });
  });
  ...
  it('should update the declaration status successfully', async () => {
    ...
    jest
      .spyOn(declarationRepository, 'getDeclarationById')
      .mockResolvedValue(existingDeclaration);

    await declarationService.updateDeclarationStatus(
      declarationId,
      declaration
    );

    expect(updateDeclarationSpy).toHaveBeenCalledWith(
      declarationId,
      declaration
    );
  });
  ...
  describe('get a declaration ', () => {
    ...
    it('should get and return declaration of teamMate 1', async () => {
      jest
        .spyOn(declarationRepository, 'getDeclarationForTeamMate')
        .mockResolvedValueOnce(declarationDtoIn);

      const declarationForTeamMates =
        await declarationService.getDeclarationForTeamMate(1);
